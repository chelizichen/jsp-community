...
<div class="right">
    <div class="location">
        <strong>你现在所在的位置是:</strong> <span>房屋管理页面</span>
    </div>
    <div class="search">
        <form method="get" action="${pageContext.request.contextPath }/house?opr=list"
              id="queryform">
            ...
            <a href="${pageContext.request.contextPath}/house?opr=update">添加房屋</a>
        </form>
    </div>
    <!--用户-->
    <table class="providerTable" cellpadding="0" cellspacing="0">
        ...
        <c:forEach var="house" items="${requestScope.houseList }" varStatus="status">
            <tr>
                <td><span>${house.id }</span>
                </td>
                <td><span>${house.address }</span>
                </td>
                <td>
                    <span>
                    <c:if test="${house.type== '1'}">商品房</c:if>
                    <c:if test="${house.type== '2'}">公寓</c:if>
                    </span>
                </td>

                <td><span>${house.area}</span></td>
                <td><span>${house.price}</span></td>
                <td><span>${house.name}</span></td>

                <td><span><a class="viewUser" href="javascript:goHouseView(${house.id});"
                             userid=${house.id } username=${house.name }>
...
<%--						    </a> </span> <span>--%>
							...
							<a class="deleteVistor" userid=${house.id } username=${house.name }
                               href="#">
...
<script type="text/javascript">
    ...
    function goHouseView(id) {
        window.location.href = "${pageContext.request.contextPath}/house?opr=update&id="+id
    }
