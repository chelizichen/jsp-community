...
<script>
    function gotoPage(p) {
        ...
        window.location.href=
            "${pageContext.request.contextPath}/jsp/bill.do?pageIndex="+p+"&"+$("#billFrom").serialize();
    };
</script>
<div class="right">
       <div class="location">
       ...
       <div class="search">
       <form id="billFrom" method="get" action="${pageContext.request.contextPath}/jsp/bill.do">
			...
			<select name="queryProviderId">
				<c:if test="${providerList != null }">
				   ...
				   <c:forEach var="provider" items="${providerList}">
				   		<option <c:if test="${provider.id eq queryProviderId}">selected="selected"</c:if>
			...
			<select name="queryIsPayment">
				<option value="0">--请选择--</option>
				<option value="1" ${queryIsPayment == 1 ? "selected":"" }>未付款</option>
				<option value="2" ${queryIsPayment == 2 ? "selected":"" }>已付款</option>
       		</select>
			
			 <input	value="查 询" type="submit" id="searchbutton">
			 <a href="${pageContext.request.contextPath }/jsp/gotoBillAdd.do">添加订单</a>
		</form>
       </div>
       <!--账单表格 样式和供应商公用-->
	...
	<center>
		<div style="margin-top:10px">
			...
			<a href="javascript:gotoPage(${pageInfo.pageNum-1})">上一页</a>
			<c:if test="${not empty pageInfo.pages}">
				<c:forEach begin="1" end="${pageInfo.pages}" step="1" var="s">
					<a href="javascript:gotoPage(${s})">${s}</a>
				</c:forEach>
			</c:if>

			<a href="javascript:gotoPage(${pageInfo.pageNum + 1 })">下一页</a>
			<a href="javascript:gotoPage(${pageInfo.pages})">尾页</a>
			第<input type="text" style="width:20px" id="index" name="index"/>页
			<input type="button" value="确定"
			       onclick="gotoPage(document.getElementById('index').value)"></button>
		</div>
	</center>
</section>
