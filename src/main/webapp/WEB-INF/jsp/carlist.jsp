...
<div class="right">
    <div class="location">
        <strong>你现在所在的位置是:</strong> <span>车辆管理页面</span>
    </div>
    <div class="search">
        <form method="get" action="${pageContext.request.contextPath }/car?opr=list"
              id="queryform">
            ...
            <a href="${pageContext.request.contextPath}/car?opr=update">添加车辆</a>
        </form>
    </div>
    <!--用户-->
    <table class="providerTable" cellpadding="0" cellspacing="0">
        ...
        <c:forEach var="Car" items="${requestScope.carList }" varStatus="status">
            <tr>
                ...
                <td>
                    <span>
                    <c:if test="${Car.io=='1'}">入</c:if>
                    <c:if test="${Car.io=='2'}">出</c:if>
                    </span>
                </td>
                <td><span>${Car.iotime}</span>
                </td>
                <td><span>${Car.username}</span></td>
                <td><span>${Car.paytype}</span></td>

                <td><span><a class="viewUser" href="javascript:goViewCar(${Car.id});"
                             userid=${Car.id } username=${Car.username }>
...
<%--						    </a> </span> <span>--%>
							...
							<a class="deleteCar" userid=${Car.id } username=${Car.id }
                               href="#">
...
<script type="text/javascript">
    ...
    // }
    // }
    function goViewCar(id) {
        window.location.href = "${pageContext.request.contextPath}/car?opr=update&id="+id
    }
